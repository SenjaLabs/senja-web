"use client";

import Image from "next/image";
import { ComponentProps } from 'react';

export interface LogoImageProps extends Omit<ComponentProps<typeof Image>, 'src' | 'alt'> {
  size?: number;
  alt?: string;
  responsive?: boolean;
}

export const LogoImage = ({ 
  size = 200, 
  alt = "Senja Logo", 
  className = "",
  responsive = true,
  ...props 
}: LogoImageProps) => {
  const responsiveClasses = responsive 
    ? "w-32 h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 xl:w-56 xl:h-56" 
    : "";
  
  return (
    <Image
      src="/beary/beary.png"
      alt={alt}
      width={size}
      height={size}
      className={`object-contain ${responsive ? responsiveClasses : ''} ${className}`}
      {...props}
    />
  );
};

// Keep original Logo component for backward compatibility (fallback to SVG)
export type LogoProps = ComponentProps<'svg'>;

export const Logo = (props: LogoProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44" fill="none" {...props}>
    <path
      d="M25.8665 14.41C19.6515 10.0485 11.88 10.417 8.50299 15.2295C5.12599 20.0365 7.43049 27.4725 13.6455 31.8285C15.543 33.1595 17.5835 34.0505 19.602 34.507C19.1455 35.53 18.4415 36.476 17.7265 37.2625C17.116 37.9335 17.7925 38.984 18.656 38.72C26.983 36.1845 30.547 31.6085 30.547 31.6085C30.7065 31.4215 30.8605 31.2235 31.009 31.0145C34.386 26.2075 32.0815 18.7715 25.8665 14.4155V14.41ZM11.9845 17.666C12.1605 17.4185 12.3585 17.193 12.573 16.9895C12.716 16.8575 12.936 16.962 12.936 17.1545C12.859 19.921 14.696 23.2045 17.9025 25.5145C21.23 27.9125 25.08 28.5835 27.687 27.478C27.863 27.4065 28.039 27.5825 27.962 27.7585C27.8465 28.039 27.6925 28.314 27.5165 28.567C25.4925 31.4545 20.372 31.3555 16.082 28.3415C11.7865 25.333 9.95499 20.5535 11.979 17.666H11.9845Z"
      fill="currentColor"
    />
    <path
      d="M35.4807 24.9094L35.3982 25.0304C35.2387 25.2559 34.8867 25.1459 34.8867 24.8709C34.8647 23.0449 34.5457 21.23 33.9187 19.5085C33.3412 17.941 32.5162 16.467 31.4932 15.147C30.8002 14.2505 30.0137 13.4145 29.1502 12.683C28.0447 11.748 26.8017 10.9395 25.4872 10.3235C23.9252 9.59196 22.2532 9.10246 20.5427 8.88796C18.1887 8.58546 15.7852 8.82746 13.5247 9.56996C13.4367 9.59746 13.3542 9.62496 13.2717 9.65796C13.0132 9.74596 12.7932 9.44896 12.9527 9.22896L13.0352 9.11346C16.4067 4.30646 24.1837 3.94346 30.3987 8.29946C36.5972 12.683 38.8742 20.1245 35.4862 24.9204L35.4807 24.9094Z"
      fill="currentColor"
    />
  </svg>
);
