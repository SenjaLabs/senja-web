  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/3 -right-10 w-32 h-32 bg-white/5 rounded-full animate-bounce" style={{ animationDelay: "1s" }}></div>
        <div className="absolute -bottom-10 left-1/3 w-24 h-24 bg-white/10 rounded-full animate-pulse" style={{ animationDelay: "0.5s" }}></div>
        
        {/* Gradient orbs - adjusted colors to match peach-coral theme */}
        <div className="absolute top-1/4 left-1/4 w-20 h-20 bg-gradient-to-r from-orange-300 to-pink-400 rounded-full opacity-20 animate-ping" style={{ animationDelay: "2s" }}></div>
        <div className="absolute bottom-1/4 right-1/4 w-16 h-16 bg-gradient-to-r from-pink-400 to-purple-500 rounded-full opacity-30 animate-ping" style={{ animationDelay: "1.5s" }}></div>
      </div>

      {/* Main content */}
      <div className="relative flex flex-col items-center space-y-6 md:space-y-8 lg:space-y-10 z-10">
        {/* Logo with enhanced animation */}
        <div className={`transform transition-all duration-1000 ease-out ${logoAnimated && logoLoaded ? 'animate-bounce scale-100' : 'scale-75 opacity-0'}`}>
          <div className="bg-white/25 backdrop-blur-sm rounded-3xl p-6 md:p-8 lg:p-10 shadow-2xl border border-white/40">
            {logoLoaded ? (
              <Image
                ...
                onError={() => console.warn("Logo failed to display")}
              />
            ) : (
        ...
        <div className="text-center space-y-2 md:space-y-3">
          ...
          <p className="text-base md:text-lg lg:text-xl text-white/90 animate-fade-in-up drop-shadow-md" style={{ animationDelay: "0.3s" }}>
            Crosschain Lending Protocol
          </p>
        </div>

        {/* Enhanced loading indicator */}
        <div className="flex space-x-2 md:space-x-3 lg:space-x-4 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
          <div className="w-2 h-2 md:w-3 md:h-3 lg:w-4 lg:h-4 bg-white/70 rounded-full animate-bounce"></div>
          <div className="w-2 h-2 md:w-3 md:h-3 lg:w-4 lg:h-4 bg-white/70 rounded-full animate-bounce" style={{ animationDelay: "0.1s" }}></div>
          <div className="w-2 h-2 md:w-3 md:h-3 lg:w-4 lg:h-4 bg-white/70 rounded-full animate-bounce" style={{ animationDelay: "0.2s" }}></div>
        </div>
      </div>

      {/* Bottom content */}
      <div className="absolute bottom-16 left-0 right-0 text-center animate-fade-in-up" style={{ animationDelay: "1s" }}>
