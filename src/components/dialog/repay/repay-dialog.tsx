...
/**
 * Props for the RepayDialog component
 */
...
/**
 * RepayDialog component for repaying debt to a lending pool
 * 
 * @param props - Component props
 * @returns JSX element
 */
export const RepayDialog = memo(function RepayDialog({
  isOpen,
  onClose,
  pool,
  onSuccess,
      ...
      <DialogContent className={`${dialogStyles.content} ${className || ''}`}>
        ...
        <div className="px-6 py-6">
          <form onSubmit={handleSubmit} className={spacing.form}>
            {/* Pool Information */}
            <Card className="p-4 bg-gradient-to-r from-indigo-50 to-indigo-100 border border-indigo-200 rounded-xl mb-6">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <div className="text-gray-500">Borrow Token:</div>
                  <div className="font-semibold text-indigo-700">{pool.borrowTokenInfo?.symbol}</div>
                </div>
                <div>
                    ...
                    <div className="flex items-center gap-3 text-blue-600">
                      <div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                       <span className="text-sm font-semibold">
                         {LOADING_MESSAGES.REPAYING || "Repaying debt..."}
                       </span>
                    </div>
                  )}
                  ...
                  {isSuccess && (
                    <div className="flex items-center gap-3 text-green-600">
                      <div className="w-5 h-5 bg-green-600 rounded-full flex items-center justify-center">
                       ...
                       <span className="text-sm font-semibold">
                         {SUCCESS_MESSAGES.REPAY_SUCCESSFUL || "Repay successful!"}
                       </span>
                    </div>
                  )}
