...
/**
 * Props for the RepayByCollateralDialog component
 */
interface RepayByCollateralDialogProps {
  ...
  /** Callback when repay is successful */
  onSuccess?: () => void;
  /** Custom className */
  className?: string;
}
...
/**
 * RepayByCollateralDialog component for repaying debt using collateral
 * 
 * @param props - Component props
 * @returns JSX element
 */
export const RepayByCollateralDialog = memo(function RepayByCollateralDialog({
  isOpen,
  onClose,
  pool,
  onSuccess,
  className,
}: RepayByCollateralDialogProps) {
  ...
  const handleClose = useCallback(() => {
    if (!isRepaying && !isConfirming) {
      onClose();
      resetForm();
    }
  }, [isRepaying, isConfirming, onClose, resetForm]);
      ...
      <DialogContent className={`${dialogStyles.content} ${className || ''}`}>
        ...
        <div className="px-6 py-6">
          <form onSubmit={handleSubmit} className={spacing.form}>
            {/* Pool Information */}
            <Card className="p-4 bg-gradient-to-r from-teal-50 to-teal-100 border border-teal-200 rounded-xl mb-6">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <div className="text-gray-500">Collateral Token:</div>
                  <div className="font-semibold text-teal-700">{pool.collateralTokenInfo?.symbol}</div>
                </div>
                <div>
            ...
            <div className="space-y-3">
              ...
              <div className="text-xs text-gray-500">
                This will use your collateral to repay your debt
              </div>
            </div>

            {/* Transaction Status */}
            {(isRepaying || isConfirming || isSuccess || isError) && (
                    ...
                    <div className="flex items-center gap-3 text-blue-600">
                      <div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                       <span className="text-sm font-semibold">
                         {LOADING_MESSAGES.REPAYING || "Repaying by collateral..."}
                       </span>
                    </div>
                  )}
                  ...
                  {isSuccess && (
                    <div className="flex items-center gap-3 text-green-600">
                      <div className="w-5 h-5 bg-green-600 rounded-full flex items-center justify-center">
                       ...
                       <span className="text-sm font-semibold">
                         {SUCCESS_MESSAGES.REPAY_BY_COLLATERAL_SUCCESSFUL || "Repay by collateral successful!"}
                       </span>
                    </div>
                  )}
